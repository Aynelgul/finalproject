...
- Nieuwe sketches toegevoegd aan DESIGN.md (advanced, in programma gemaakt).
...
- Aanpassen MapViewController op Main.Storyboard: andere vormgeving, paste beter. Formulieren anders vormgeven: met segmented control voor tips/travels (overzichtelijker).
- Structs aanmaken (nog niet af), begonnen met user.
...
## Vrijdag, 27 jan 2017
- Op aanraden van Julian geen globale functies gebruikt, maar wel bijvoorbeeld alle 'alerts' per view in een functie gedaan. Dit betekent wel dat de alert function meerdere keren opnieuw wordt 'aangemaakt' in verschillende views.

## Maandag, 30 jan 2017
- Beetje opschonen code: zo veel mogelijk functies uit ViewDidLoad; alleen aanroepen van functies.
- Bezig geweest met toevoegen buddy in Firebase. Ging eerst mis: hij bleef dezelfde buddy toevoegen (infinite loop). Kwam uiteindelijk door een 'observer' die dus elke keer een verandering observede als je een buddy toevoegde en dan elke keer weer de buddy ging toevoegen. Uiteindelijk opgelost door observeSingleEvent.
...
- Opgeschoond: functie aangemaakt voor converting NSDate naar String.
...
- Aan REPORT.md gewerkt: duidelijke schets van de viewControllers en dergelijke.
- README.md aangepast naar huidige app.
- Ongebruikte plaatjes uit Assets.xcassets gehaald.
